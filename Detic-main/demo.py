...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            # if args.output is none, then it will be the same as input
            if args.output is None:
                args.output = "/".join(path.split('/')[:-1])
            predictions, visualized_output = demo.run_on_image(img,args.output,image_path=path)
            # print(predictions)
