...
try:
    with open('/media/gklpcsgn/CE623CD9623CC84B/TYX/catagories.json') as f:
...
class Detector:
    ...
    def onImage(self,imagePath):
        ...
        # if there is no folder named detections, create one
        if not os.path.exists("detections"):
            os.makedirs("detections")
        # we will have a folder for each image in the detections folder
        # if there is no folder named detections/image_name, create one. If there is, add _1, _2, _3, etc.
        if not os.path.exists("detections/"+imagePath.split("/")[-1].split(".")[0]):
            save_path = "detections/"+imagePath.split("/")[-1].split(".")[0]
            os.makedirs(save_path)
        else:
            i = 1
            while os.path.exists("detections/"+imagePath.split("/")[-1].split(".")[0]+"_"+str(i)):
                i += 1

            save_path = "detections/"+imagePath.split("/")[-1].split(".")[0]+"_"+str(i)
            os.makedirs(save_path)
